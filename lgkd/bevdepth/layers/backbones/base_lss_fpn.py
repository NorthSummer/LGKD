...
class Fine_DepthNet(nn.Module):
    def __init__(self, in_channels=None, mid_channels=None, non_negative=True, scale=1.0, shift=0.0, invert=False):
        super().__init__()
        ...
        self.fine_depth_pred = nn.Sequential(
        ...
        self.head = nn.Sequential(  
            nn.Conv2d(mid_channels, 32, kernel_size=1, stride=1, padding=0),         
            #Interpolate(scale_factor=4, mode="bilinear", align_corners=True),   
    ...
    def forward(self, x):
        ...
        inv_depth_x = self.fine_depth_pred(x)
        inv_depth = self.head(inv_depth_x)


        if self.invert:
            depth = self.scale * inv_depth + self.shift
            depth[depth < 1e-3] = 1e-3
            depth[depth > 256.0] = 256.0
            depth = 1.0 / depth
            return depth
        else:
            return inv_depth
...
class BaseLSSFPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.fine_depth_net = Fine_DepthNet(512, 128, non_negative=True, scale=1.0, shift=0.0, invert=False)
    ...
    def _forward_depth_net(self, feat, mats_dict):
        depth_dis = self.depth_net(feat, mats_dict)
        depth_fine = self.fine_depth_net(feat)
        return depth_dis, depth_fine
        #return depth_dis
    ...
    def _forward_single_sweep(self,
        ...
        source_feature = source_features.reshape(batch_size * num_cams,
                                    source_features.shape[2],
                                    source_features.shape[3],
                                    source_features.shape[4])

        depth_feature, fine_depth = self._forward_depth_net( #, fine_depth
            source_feature,
            mats_dict,
          )        
        ...
        if self.training or self.use_da:
            ...
            if self.teacher:
                ...
                view_1 = voxel_pooling_inference(
                    geom_xyz[0,0,:,:,:,:].unsqueeze(0).contiguous(), depth, depth_feature[0, self.depth_channels:(
                        self.depth_channels + self.output_channels)].contiguous(),
                    self.voxel_num.cuda()) 
                    
                #view_1 = torch.sum(view_1, dim=1) 
                
    
                view_2 = voxel_pooling_inference(
                    geom_xyz[0,1,:,:,:,:].unsqueeze(0).contiguous(), depth, depth_feature[1, self.depth_channels:(
                        self.depth_channels + self.output_channels)].contiguous(),
                    self.voxel_num.cuda())  
                    
                view_3 = voxel_pooling_inference(
                    geom_xyz[0,2,:,:,:,:].unsqueeze(0).contiguous(), depth, depth_feature[2, self.depth_channels:(
                        self.depth_channels + self.output_channels)].contiguous(),
                    self.voxel_num.cuda())                                  
                    
                view_4 = voxel_pooling_inference(
        ...
        if is_return_depth and self.teacher:
            ...
            return feature_map.contiguous(
            ), depth_feature[:, :self.depth_channels].softmax(dim=1), fine_depth
